...
class PTagCloud
{
	...
	function __construct($displayedElementCount, $arSeedWords = false)
	{
	    ...
	    $this->m_searchURL = "search/pattern,";
	    $this->m_bUTF8 = false;
	    $this->m_backgroundColor = "#fff";
	    
	    $this->m_arColors[0] = "#999";
	    $this->m_arColors[1] = "#999";
	    $this->m_arColors[2] = "#808080";
	    $this->m_arColors[3] = "#808080";
	    $this->m_arColors[4] = "#666";
	    $this->m_arColors[5] = "#666";
	    $this->m_arColors[6] = "#333";
	    $this->m_arColors[7] = "#333";
	    $this->m_arColors[8] = "#000";
	    $this->m_arColors[9] = "#000";
    ...
    function keywords_extract($text)
    {
        ...
        $commonWords = "'tis,'twas,a,able,about,across,after,ain't,all,almost,also,am,among,an,and,any,are,aren't," .
            "as,at,b,be,because,been,but,by,c,can,can't,cannot,could,could've,couldn't,d,dr,dear,did,didn't,do,does,doesn't," .
            "don't,either,else,ever,every,e,f,for,from,get,g,got,h,had,has,hasn't,have,he,he'd,he'll,he's,her,hers,him,his," .
            "how,how'd,how'll,how's,however,i,i'd,i'll,i'm,i've,if,in,into,is,isn't,it,it's,its,j,just,k,l,least,let,like," .
            "likely,may,me,might,might've,mightn't,most,must,must've,mustn't,my,m,n,neither,no,nor,not,o,o'clock,of,off," .
            ...
            "you're,you've,p,q,,r,t,u,v,w,x,y,z,title,pakhtunkhwa,your";
	...
	function listcloud_Show($bHTML = true)
	{
		...
		if (is_array($this->m_arTags))
		{
			if ($bHTML) 
			    $result = '<div id="id_tag_cloud" style="' . (isset($this->m_width) ? ("width:". $this->m_width. ";") : "") .
			               'line-height:normal"><div style="border-style:none;border-width:0px;' .
			              (isset($this->m_backgroundImage) ? ("background:url('". $this->m_backgroundImage ."');") : "") .
			              'margin-top:20px;margin-bottom:10px;padding:5px 5px 20px 5px;background-color:'.$this->m_backgroundColor.';">';
            else			    
                $result = array();
			foreach ($arTopTags as $tag => $useCount)
			{
				...
				if ($bHTML)
				{
					$result .= ("<a href=\"". $this->m_searchURL.urlencode($tag)."\" title=\"More info on " .
					    ...
					    "padding:4px; font-family:Arial,sans-serif; font-weight:900; font-size:" . 
					    (0.3 * $grade) . "em\">".$tag."</span></a> ");
				}
				else
				    $result[$tag] = $grade;
			}
